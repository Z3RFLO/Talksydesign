...
export default function Explore() {
  ...
  return (
    <div className="w-full max-w-[1000px] mx-auto pt-10 pb-32 animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      <div className="mb-12">
        <h1 className="text-5xl font-black text-white mb-2 tracking-tight">Explore</h1>
        <p className="text-gray-500 text-lg">Find your frequency in the noise.</p>
      </div>
      ...
      <div className="relative mb-16 group">
        ...
        <div className="relative bg-[#0a0a0a] border border-white/10 rounded-2xl flex items-center px-6 py-5 transition-all group-focus-within:border-white/30">
          ...
          <input type="text" placeholder="Search..." className="w-full bg-transparent text-xl text-white outline-none border-none" />
        </div>
      </div>

      {/* Featured Cards */}
      <h3 className="text-2xl font-bold text-white mb-6 flex items-center gap-3"><Zap className="text-yellow-400" fill="currentColor" /> Featured</h3>
        ...
        {FEATURED_CARDS.map((card, i) => (
          <div key={i} onClick={() => navigate(`/explore/${card.tag}`)} className="h-40 rounded-3xl relative overflow-hidden group cursor-pointer border border-white/5 hover:border-white/20 transition-all active:scale-95">
            ...
            <div className="absolute bottom-6 left-6">
              ...
              <div className="text-2xl font-black text-white group-hover:translate-x-2 transition-transform">{card.title}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Trending Tags */}
      <h3 className="text-2xl font-bold text-white mb-8 flex items-center gap-3"><TrendingUp className="text-[#f91880]" /> Trending Now</h3>
