...
import { X, MapPin, User as UserIcon, Languages, MessageCircle, UserPlus, CheckCircle, ArrowRight, LucideIcon } from 'lucide-react';
import { Link } from 'react-router-dom';

// Define a type for the user object passed to this component
// It needs to support both Post author and potential User object
interface UserProfileProps {
    author?: string; // Post object has author
    username?: string; // User object has username
    avatar?: string;
    handle?: string;
}
...
const USER_DETAILS: Record<string, UserDetailStats> = {
...
const InfoPill: React.FC<InfoPillProps> = ({ icon: Icon, text, color }) => (
...
export default function UserPopup({ user, onClose }: UserPopupProps) {
    if (!user) return null;

    // Determine the display name (author or username)
    const displayName = user.author || user.username || "Unknown";

    const details = USER_DETAILS[displayName] || { bio: "Traveler of the void.", city: "Unknown", gender: "Unknown", lang: "Universal", connections: "0", posts: "0" };
            ...
            >
                ...
                <div className="bg-black/40 rounded-[42px] p-8 relative overflow-hidden h-full">
                    ...
                    <div className="flex justify-center gap-2 mt-6 flex-wrap">
                        ...
                        <InfoPill icon={UserIcon} text={details.gender} color="text-pink-400" />
