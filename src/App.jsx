...
import { Plus, MessageCircle, X } from 'lucide-react'; // Updated Icons
...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex justify-center bg-black relative">
      ...
      {/* Hide Sidebar on Messages page for cleaner look (Optional, but looks better) */}
      ...
      <main className={`w-full ${!isMessagesPage ? 'pl-[120px] pr-10' : 'px-4'}`}>
        <Routes>
          ...
          <Route path="/settings" element={<div className="text-white pt-40 text-center text-2xl">Settings</div>} />
        </Routes>
      </main>

      {/* --- SMART BUTTON (Top Right) --- */}
      <Link 
        to={isMessagesPage ? "/" : "/messages"} // Logic: If on Msg, go Home. If Home, go Msg.
        className="fixed top-8 right-8 z-50"
      >
        <div className={`w-14 h-14 backdrop-blur-xl border rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 shadow-2xl group
          ${isMessagesPage 
            ? 'bg-white border-white text-black hover:rotate-90 hover:bg-gray-200' // Style for "Close"
            : 'bg-[#0c0c0e]/80 border-white/10 text-white hover:bg-white hover:text-black hover:scale-110' // Style for "Chat"
          }`}
        >
